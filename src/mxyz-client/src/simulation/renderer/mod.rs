...
impl Renderer {
    ...
    pub fn display(&mut self, engine: &Engine) {
        ...
        if self.config.is_clearing_canvas {
            canvas.clear();
        }
            ...
            for col_idx in 0..dimensions[1] {
                ...
                for family in families.iter() {
                    ...
                    for obj_id in 0..nr_of_objects {
                        let obj =
                            Vec::from(&objects[obj_id * obj_length..(obj_id + 1) * obj_length]);
                        ...
                        let force_getter = match self.sim_id.as_str() {
                            ...
                            "electric-field" => obj_obj_forces::coulomb::force,
                            _ => obj_obj_forces::newtonian_gravity::force,
                        };

                        let f = force_getter(&[m, x, y, u, v, q], &obj, eps);
                        force[0] += f[0];
                        force[1] += f[1];
                    }
                }
                ...
                let from = (x, y);
                let to = (
                    x + force[0] / norm / dimensions[0] as f64,
                    y + force[1] / norm / dimensions[1] as f64,
                );
                let radius = 0.001;
    ...
    pub fn display_line_tails(
        ...
        for tail_step_id in (0..usize::min(tail_length, frame_idx)).rev() {
            for obj_id in 0..nr_of_objects {
                ...
                let previous_obj = &previous_state.obj_families[family.id].objects
                    [start_idx..start_idx + obj_length];
    ...
    pub fn display_info_textfields(&mut self, engine: &Engine) {
        let doc = mxyz_utils::dom::document();
    ...
    pub fn display_hud(&mut self, _engine: &Engine) {
