...
impl State {
    pub fn new(sim_id: &str, engine_conf: &mut EngineConfig) -> Self {
        State {
    ...
    pub fn setup_objects(sim_id: &str, engine_conf: &mut EngineConfig) -> Vec<ObjFamily> {
        ...
        match sim_id {
            ...
            "3body-fig8" => {
                ...
                let us = [
                    -0.06507160612095737318,
                    0.1301432122419148851,
                    -0.06507160612095737318,
                ];
                let vs = [
                    0.6324957346748190101,
                    -1.264991469349638020,
                    0.6324957346748190101,
                ];
            ...
            "nbody-misc" => {
                ...
                let (mass, distance, speed, nr_of_objects) = (0.05, 0.4, 0.9, 8);
            ...
            "charge-interaction_2" => {
                ...
                let nr_of_electrons = 36;
                for _body_idx in 0..nr_of_electrons {
                    let rand1: f64 = rng.gen();
                    let rand2: f64 = rng.gen();
                    let x0 = rand1 * 2. - 1.;
                    let y0 = rand2 * 2. - 1.;
                    let object = Vec::from([0.05, x0, y0, 0., 0., 1.]);
                    family.add_object(&object, &mut conf);
                }
                engine_conf.obj_families.push(conf);
                obj_families.push(family);
            ...
            "gravity-field" => {
                ...
                let mut conf = ObjFamilyEngineConfig::new(id);
                conf.obj_variant = ObjVariant::Static;
                conf.obj_interactions = Vec::from([ObjObjInteraction::ForceNewtonianGravity]);
                conf.boundary = ObjBoundaryVariant::WallCollisionElastic;
                conf.obj_attributes.push(ObjAttribute::Mass);
                conf.obj_length += 1;

                let x0 = -0.5;
                let y0 = 0.;
                let object = Vec::from([0.4, x0, y0, 0., 0., 0.]);
                family.add_object(&object, &mut conf);

                let x0 = 0.5;
                let y0 = 0.;
                let object = Vec::from([1.2, x0, y0, 0., 0., 0.]);
                family.add_object(&object, &mut conf);
            ...
            "electric-field" => {
                ...
                let x0 = -0.5;
                let y0 = 0.;
                let object = Vec::from([0.5, x0, y0, 0., 0., -1.]);
                family.add_object(&object, &mut conf);
    ...
    pub fn setup_fields(sim_id: &str, engine_conf: &mut EngineConfig) -> Vec<Field> {
...
pub fn get_initial_state_for_GOL_glider(initial_position: (usize, usize)) -> Vec<(usize, usize)> {
