...
const img_spaceship = new Image();
img_spaceship.src = "/static/media/spaceship_off.png";
...
const player = {
  ...
  height: 60,
  applyGravity: () => {
    let d = Math.sqrt((player.x - sun.x) ** 2 + (player.y - sun.y) ** 2);
    let a = (1 * sun.mass) / d ** 2;
    let u = [[(sun.x - player.x) / d], [(sun.y - player.y) / d]];
    let a_x = a * u[0];
    let a_y = a * u[1];
    player.v_x += a_x / fps;
    player.v_y += a_y / fps;
  ...
  move: () => {
    player.x += player.v_x / fps;
    player.y += player.v_y / fps;
  },
};
...
window.addEventListener("keydown", function (e) {
  if (e.key == " ") {
    img_spaceship.src = "/static/media/spaceship_on.png";
    player.v_y -= 0.1;
  }
});
window.addEventListener("keyup", function (e) {
...
function animate() {
  ...
  if (elapsed > fpsInterval) {
    ...
    drawImage(img_spaceship, player.x, player.y, player.width, player.height);
    player.applyGravity();
    player.move();
